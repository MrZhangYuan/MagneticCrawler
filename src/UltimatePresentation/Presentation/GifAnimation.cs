...
namespace UltimatePresentation.Presentation
{
	...
	class GifAnimation : Viewbox, IDisposable
	{
		...
		public void CreateGifAnimation(MemoryStream memoryStream)
		{
			...
			{
				StartParseGif();
				byte[] data = memoryStream.GetBuffer();  // Use GetBuffer so that there is no memory copy
		    ...
		    for (int i = 0; i < frameList.Count; i++)
				{
			    // Console.WriteLine(i + " " + frameList[i].disposalMethod.ToString() + " " + frameList[i].width.ToString() + " " + frameList[i].delayTime.ToString());
		    }
#endif
