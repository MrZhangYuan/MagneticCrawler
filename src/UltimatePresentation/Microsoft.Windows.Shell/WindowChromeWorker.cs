...
namespace Microsoft.Windows.Shell
{
    ...
    internal class WindowChromeWorker : DependencyObject
    {
        ...
        private void _ApplyNewCustomChrome()
        {
            ...
            NativeMethodsShell.SetWindowPos(_hwnd, IntPtr.Zero, 0, 0, 0, 0, _SwpFlags);
        }
        ...
        private void _FixupTemplateIssues()
        {
            ...
            if (_IsPresentationFrameworkVersionLessThan4)
            {
                RECT rcWindow = NativeMethodsShell.GetWindowRect(_hwnd);
        ...
        private void _FixupRestoreBounds(object sender, EventArgs e)
        {
            ...
            if (_window.WindowState == WindowState.Maximized || _window.WindowState == WindowState.Minimized)
            {
                ...
                {
                    _hasUserMovedWindow = false;
                    WINDOWPLACEMENT wp = NativeMethodsShell.GetWindowPlacement(_hwnd);
        ...
        private RECT _GetAdjustedWindowRect(RECT rcWindow)
        {
            ...
            var style = (WS)NativeMethodsShell.GetWindowLongPtr(_hwnd, GWL.STYLE);
            var exstyle = (WS_EX)NativeMethodsShell.GetWindowLongPtr(_hwnd, GWL.EXSTYLE);

            return NativeMethodsShell.AdjustWindowRectEx(rcWindow, style, false, exstyle);
        }
        ...
        private IntPtr _HandleSetTextOrIcon(WM uMsg, IntPtr wParam, IntPtr lParam, out bool handled)
        {
            ...
            IntPtr lRet = NativeMethodsShell.DefWindowProc(_hwnd, uMsg, wParam, lParam);
        ...
        private IntPtr _HandleNCActivate(WM uMsg, IntPtr wParam, IntPtr lParam, out bool handled)
        {
            ...
            IntPtr lRet = NativeMethodsShell.DefWindowProc(_hwnd, WM.NCACTIVATE, wParam, new IntPtr(-1));
            handled = true;
            return lRet;
        }
        ...
        private IntPtr _HandleNCHitTest(WM uMsg, IntPtr wParam, IntPtr lParam, out bool handled)
        {
            ...
            if (_chromeInfo.UseAeroCaptionButtons)
            {
                ...
                if (Utility.IsOSVistaOrNewer && _chromeInfo.GlassFrameThickness != default(Thickness) && _isGlassEnabled)
                {
                    ...
                    handled = NativeMethodsShell.DwmDefWindowProc(_hwnd, uMsg, wParam, lParam, out lRet);
...
//                }
        ...
        {
            Assert.IsNotDefault(_hwnd);
            var dwStyle = (WS)NativeMethodsShell.GetWindowLongPtr(_hwnd, GWL.STYLE).ToInt32();
            ...
            NativeMethodsShell.SetWindowLongPtr(_hwnd, GWL.STYLE, new IntPtr((int)dwNewStyle));
            return true;
        }
        ...
        {
            var wpl = NativeMethodsShell.GetWindowPlacement(_hwnd);
            switch (wpl.showCmd)
            {
        ...
        private Rect _GetWindowRect()
        {
            ...
            RECT windowPosition = NativeMethodsShell.GetWindowRect(_hwnd);
        ...
        private void _UpdateSystemMenu(WindowState? assumeState)
        {
            ...
            if (null != assumeState || _lastMenuState != state)
            {
                ...
                IntPtr hmenu = NativeMethodsShell.GetSystemMenu(_hwnd, false);
                if (IntPtr.Zero != hmenu)
                {
                    ...
                    switch (state)
                    {
                        case WindowState.Maximized:
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.RESTORE, mfEnabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MOVE, mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.SIZE, mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MINIMIZE, canMinimize ? mfEnabled : mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MAXIMIZE, mfDisabled);
                            break;
                        case WindowState.Minimized:
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.RESTORE, mfEnabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MOVE, mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.SIZE, mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MINIMIZE, mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MAXIMIZE, canMaximize ? mfEnabled : mfDisabled);
                            break;
                        default:
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.RESTORE, mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MOVE, mfEnabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.SIZE, canSize ? mfEnabled : mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MINIMIZE, canMinimize ? mfEnabled : mfDisabled);
                            NativeMethodsShell.EnableMenuItem(hmenu, SC.MAXIMIZE, canMaximize ? mfEnabled : mfDisabled);
                            break;
                    }
        ...
        private void _UpdateFrameState(bool force)
        {
            ...
            bool frameState = NativeMethodsShell.DwmIsCompositionEnabled();
        ...
        {
            NativeMethodsShell.SetWindowRgn(_hwnd, IntPtr.Zero, NativeMethodsShell.IsWindowVisible(_hwnd));
        }
        ...
        private void _SetRoundingRegion(WINDOWPOS? wp)
        {
            ...
            WINDOWPLACEMENT wpl = NativeMethodsShell.GetWindowPlacement(_hwnd);
            ...
            if (wpl.showCmd == SW.SHOWMAXIMIZED)
            {
                ...
                IntPtr hMon = NativeMethodsShell.MonitorFromWindow(_hwnd, MONITOR_DEFAULTTONEAREST);

                MONITORINFO mi = NativeMethodsShell.GetMonitorInfo(hMon);
                ...
                try
                {
                    hrgn = NativeMethodsShell.CreateRectRgnIndirect(rcMax);
                    NativeMethodsShell.SetWindowRgn(_hwnd, hrgn, NativeMethodsShell.IsWindowVisible(_hwnd));
                    hrgn = IntPtr.Zero;
                }
                finally
                {
        ...
        private static IntPtr _CreateRoundRectRgn(Rect region, double radius)
        {
            ...
            {
                return NativeMethodsShell.CreateRectRgn(
            ...
            return NativeMethodsShell.CreateRoundRectRgn(
                (int)Math.Floor(region.Left),
                (int)Math.Floor(region.Top), 
        ...
        private static void _CreateAndCombineRoundRectRgn(IntPtr hrgnSource, Rect region, double radius)
        {
            ...
            try
            {
                ...
                CombineRgnResult result = NativeMethodsShell.CombineRgn(hrgnSource, hrgnSource, hrgn, RGN.OR);
        ...
        private void _ExtendGlassFrame()
        {
            ...
            else
            {
                ...
                NativeMethodsShell.DwmExtendFrameIntoClientArea(_hwnd, ref dwmMargin);
            }
        }
